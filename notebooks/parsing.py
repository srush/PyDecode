...
def eisner_first_order(n):
    ...
    for k in range(1, n):
        for s in range(n):
            ...
            if s != 0:
                c[Trap, Left, s, t] =                       [c.merge((Tri, Right, s, r), (Tri, Left, r+1, t),
                       values=[(t, s)])
                       for r in range(s, t)]

            c[Trap, Right, s, t] =                   [c.merge((Tri, Right, s, r), (Tri, Left, r+1, t),
                           values=[(s, t)])
                   for r in range(s, t)]

            if s != 0:
                c[Tri, Left, s, t] =                     [c.merge((Tri, Left, s, r), (Trap, Left, r, t),
                             values=[])
                     for r in range(s, t)]

            c[Tri, Right, s, t] =                   [c.merge((Trap, Right, s, r), (Tri, Right, r, t),
                           values=[])
...
#     <IPython.core.display.Image at 0x3f138d0>
