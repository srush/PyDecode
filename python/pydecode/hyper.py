...
def binarize(hypergraph):
    ...
    with new_hyper.builder() as b:
        for node in hypergraph.nodes:
            if node.is_terminal:
                d[node.id] = b.add_node(label=hypergraph.node_label(node))
                continue
            new_edges = []
            for edge in node.edges:
                if len(edge.tail) <=2:
                    new_edges.append(([d[tnode.id] for tnode in edge.tail], edge.label))
                    continue
            ...
            d[node.id] = b.add_node(new_edges, label=hypergraph.node_label(node))
    return new_hyper
...
def intersect(hypergraph, states, valid_states):
    ...
    with new_hyper.builder() as b:
        for node in hypergraph.nodes:
            print node.id
            if node.is_terminal:
                for s1, s2 in valid_states(hypergraph.node_label(node)):
                    d[node.id, s1, s2] = b.add_node(label=hypergraph.node_label(node))
            ...
            for edge in node.edges:
                if len(edge.tail) == 1:
                    for s1, s2 in node_states[edge.tail[0].id]:
                        new_edges[s1, s2].append(([d[edge.tail[0].id, s1, s2]], edge.label))
                else:
                    ...
                    for s1, s3 in node_states[edge.tail[0].id]:
                        for s2 in node_states_right[edge.tail[1].id, s3]:
                                new_edges[s1,s2].append(([d[edge.tail[0].id, s1, s3],
                                                          d[edge.tail[1].id, s3, s2]], edge.label))
            for (s1, s2), edges in new_edges.iteritems():
                d[node.id, s1, s2] = b.add_node(edges, label=hypergraph.node_label(node))
                node_states[node.id].append((s1, s2))
                node_states_right[node.id, s1].append(s2)
    return new_hyper
